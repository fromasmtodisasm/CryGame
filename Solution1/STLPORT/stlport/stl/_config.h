...
#if !defined(_STLP_DONT_USE_SHORT_STRING_OPTIM) && \
    !defined(_STLP_USE_SHORT_STRING_OPTIM)
...
/* itanium allows both settings (for instance via gcc -mbig-endian) - hence a
 * seperate check is required */
...
#if !defined(_STLP_LDOUBLE_64) && !defined(_STLP_LDOUBLE_80) && \
    !defined(_STLP_LDOUBLE_96) && !defined(_STLP_LDOUBLE_128)
...
#define _STLP_BETHREADS
    ...
    defined(N_PLAT_NLM) /* (__dest_os == __netware_os) */
...
#define _STLP_STATIC_MUTEX _STLP_mutex
    ...
    !defined(_STLP_DONT_USE_PTHREAD_SPINLOCK)
#define _STLP_USE_PTHREAD_SPINLOCK
#define _STLP_STATIC_MUTEX _STLP_mutex
#endif
...
/* When the compiler do not correctly initialized the basic types value in
 * default parameters we prefer to avoid them to be able to correct this bug.
 */
...
#if !defined(_STLP_DONT_USE_EXCEPTIONS) && !defined(_STLP_NO_EXCEPTIONS) && \
