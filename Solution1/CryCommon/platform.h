...
//
//	File: Platform.h
//	Description: Platform dependent stuff, include this file instead of
// windows.h.
//
//	History:
...
#undef assert
#define assert(exp) \
  (void)((exp) || (printf("Assert: ' %s ' has failed\n", #exp), 0))
...
#if (_MSC_VER > 1400)
#define MODERN_PLATFORM
#endif

#if defined(MODERN_PLATFORM)
...
#include <string>
#if defined(WIN64)  // || defined(LINUX64)// && defined(_DLL), be careful when
					// changing this to adapt the changes to IXml.h too
namespace cry_std {
	template <typename T>
	...
	public:
		...
		string& operator=(const Base& s) {
			...
			if (s.c_str() != this->c_str()) assign(s.c_str());
			return *this;
		}
	...
	template <typename T>
	string<T> operator+(const typename string<T>::Base& left,
		const string<T>& right) {
		return string<T>(left) += right;
	}
	template <typename T>
	string<T> operator+(const string<T>& left,
		const typename string<T>::Base& right) {
		return string<T>(left) += right;
	}
	template <typename T>
...
typedef cry_std::string<char>  // std::basic_string<char,
							   // cry_std::char_traits<char>,
							   // std::allocator<char> >
	string;
typedef cry_std::string<wchar_t>  // std::basic_string<wchar_t,
								  // cry_std::char_traits<wchar_t>,
...
#define DEFINE_ALIGNED_DATA_STATIC(type, name, alignment) \
  static type name __attribute__((aligned(alignment)));
#define DEFINE_ALIGNED_DATA_CONST(type, name, alignment) \
  const type name __attribute__((aligned(alignment)));
#else
#define DEFINE_ALIGNED_DATA(type, name, alignment) \
  _declspec(align(alignment)) type name;
#define DEFINE_ALIGNED_DATA_STATIC(type, name, alignment) \
  static _declspec(align(alignment)) type name;
#define DEFINE_ALIGNED_DATA_CONST(type, name, alignment) \
  const _declspec(align(alignment)) type name;
#endif
